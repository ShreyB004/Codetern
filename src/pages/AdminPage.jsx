...
import { Image, LayoutDashboard, LogOut, SlidersHorizontal, Users, Wrench } from 'lucide-react'
  ...
  return (
    <div className="min-h-screen bg-paper">
      <div className="flex min-h-screen">
        {/* sidebar */}
        <aside className="sticky top-0 flex h-screen w-full max-w-[260px] flex-col border-r border-ink/8 bg-ink text-white">
        ...
        <main className="flex-1 overflow-hidden">
          <div className="border-b border-ink/8 bg-white px-8 py-5">
            <h1 className="font-display text-2xl font-bold text-ink">{MODULES.find((m) => m.key === module)?.label}</h1>
            <p className="text-sm text-ink/50">{MODULES.find((m) => m.key === module)?.desc}</p>
          </div>
