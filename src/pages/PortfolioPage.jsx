            ...
            {PORTFOLIO_PROJECTS.map((project, i) => {
              ...
              return (
                <div key={project.id} data-reveal style={{ transitionDelay: `${(i % 3) * 60}ms` }}>
                  <TiltCard max={6}>
                    <article className="group flex h-full flex-col overflow-hidden rounded-panel border border-ink/8 bg-white shadow-card transition-shadow duration-500 hover:shadow-float">
                      <div className="relative h-44 overflow-hidden">
                        ...
                        <span
                          className="absolute left-4 top-4 rounded-bubble px-3 py-1 text-[10px] font-bold uppercase tracking-wider text-ink"
                      ...
                      <div className="flex flex-1 flex-col p-5">
                        <div className="flex items-start justify-between gap-2">
                          ...
                          <span className="flex shrink-0 items-center gap-1 text-[11px] font-semibold text-ink/40">
                            <Star size={11} className="fill-neon text-neon" />
                            {project.year}
                          </span>
                        </div>
                        <p className="mt-2 flex-1 text-sm leading-relaxed text-ink/55">{project.description}</p>
                        <div className="mt-3 flex flex-wrap gap-1.5">
                          {project.stack.map((s) => (
                            <span key={s} className="rounded-bubble bg-ink/5 px-2.5 py-0.5 text-[10px] font-semibold text-ink/55">
                              {s}
                            </span>
                          ))}
                        </div>
                        <div className="mt-4 flex items-center justify-between border-t border-ink/8 pt-3">
                          <p className="text-xs font-medium text-ink/45">by {project.intern}</p>
