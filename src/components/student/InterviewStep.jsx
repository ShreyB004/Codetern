    ...
    <StepShell step={3}>
      {done ? (
        <div className="flex flex-col items-center py-4 text-center">
          ...
          <h3 className="mt-5 font-display text-3xl font-bold text-ink">Milestone unlocked</h3>
          <p className="mt-2 max-w-md text-sm text-ink/55">
            Your AI scorecard: <span className="font-display text-xl font-extrabold text-ink">{candidate?.interview?.score || 84}%</span>.{' '}
          ...
          <div className="mt-5 grid w-full max-w-md gap-3 text-left">
            {Object.entries(INTERVIEW_FEEDBACK).filter(([k]) => k !== 'next').map(([k, v]) => (
              <div key={k} className="flex gap-3 rounded-2xl border border-ink/8 bg-paper px-4 py-3">
                <span className="grid h-6 w-6 shrink-0 place-items-center rounded-full bg-mint/15 text-xs font-bold text-mint">
                  <Check size={13} />
                </span>
                <div>
                  <p className="text-xs font-bold uppercase tracking-wider text-ink/45">{k}</p>
                  <p className="text-sm text-ink/70">{v}</p>
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="grid gap-8 lg:grid-cols-[1.4fr_1fr]">
          {/* camera frame */}
          <div className="relative overflow-hidden rounded-3xl border border-ink/12 bg-ink shadow-float">
          ...
          <div className="flex flex-col gap-4">
            <div className="rounded-panel border border-ink/8 bg-white p-5 shadow-card">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-sm font-bold text-ink">How this milestone works</p>
                  <p className="text-xs text-ink/45">100% simulated · no real uploads</p>
                </div>
              </div>
              <ul className="mt-4 space-y-2.5 text-sm text-ink/60">
                {['Answer on camera as the prompts auto-advance', 'Our engine scores clarity, structure & poise', 'Passing unlocks Step 4 — Booking & Seat'].map((t, i) => (
            ...
            <div className="flex-1 rounded-panel border border-dashed border-ink/15 bg-paper/60 p-5">
              <p className="text-xs font-bold uppercase tracking-widest text-ink/40">Prep notes</p>
