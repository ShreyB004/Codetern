...
import { QUIZ_BANKS, passMark } from '../../data/quizData.js'
...
export function QuizStep({ onComplete }) {
  const { candidate, saveQuizResult } = useApp()
  const { push } = useToast()
  const bank = QUIZ_BANKS[candidate?.domain]
    ...
    return (
      <StepShell step={2}>
        <p className="rounded-2xl border border-coral/30 bg-coral/8 px-4 py-3 text-sm text-coral">
          No question bank for this domain yet. Pick a different domain in Step 1.
        </p>
      </StepShell>
    )
  }

  const totalTime = bank.questions.length * 12
  ...
  const choose = (i) => {
    ...
    if (i === bank.questions[index].answer) setScore((s) => s + 1)
  }
  ...
  const next = () => {
    if (index + 1 < bank.questions.length) {
      ...
      saveQuizResult(candidate.domain, Math.round((finalScore / bank.questions.length) * 100), passed)
      setPhase('result')
  ...
  const expire = () => {
    ...
    saveQuizResult(candidate.domain, Math.round((score / bank.questions.length) * 100), passed)
    setPhase('result')
  }
    ...
    return (
      <StepShell step={2} onBack={() => {}}>
        <div className="flex flex-col items-center py-6 text-center">
          ...
          <h3 className="mt-5 font-display text-2xl font-bold text-ink">{bank.questions.length} baseline questions</h3>
          <p className="mt-2 max-w-md text-sm text-ink/55">
            You have <span className="font-bold text-ink">{totalTime}s</span> on the live clock. Score{' '}
            <span className="font-bold text-ink">{pass}+</span> to unlock the AI mock interview. No pressure — retake is
            free.
          </p>
    ...
    return (
      <StepShell step={2}>
        <div className="flex flex-col items-center py-4 text-center">
          ...
          <h3 className={cn('mt-5 font-display text-3xl font-bold', passed ? 'text-ink' : 'text-coral')}>
            {passed ? 'You passed!' : 'So close — retake it'}
          </h3>
          <p className="mt-2 text-sm text-ink/55">
            Score <span className="font-display text-2xl font-extrabold text-ink">{pct}%</span> · {score}/{bank.questions.length} correct · pass mark {pass}
          </p>
            ...
            {answered.map((a, i) => (
              <span
                ...
                className={cn(
                  ...
                  a === bank.questions[i].answer ? 'bg-mint' : 'bg-coral',
                )}
              />
            ))}
          </div>
          {passed ? (
            <Button size="lg" variant="accent" className="mt-8" onClick={onComplete}>
              Continue to mock interview <ChevronRight size={16} />
            </Button>
          ) : (
    ...
    return (
      <StepShell step={2}>
        <div className="flex flex-col items-center py-4 text-center">
          ...
          <h3 className="mt-4 font-display text-2xl font-bold text-ink">Quiz complete — {candidate.quiz.score}%</h3>
          <p className="mt-2 text-sm text-ink/55">You already cleared screening. The mock interview is next.</p>
          <Button size="lg" variant="accent" className="mt-7" onClick={onComplete}>
            Go to interview <ChevronRight size={16} />
          </Button>
        </div>
      </StepShell>
    )
  ...
  const progress = (index / bank.questions.length) * 100
    ...
    <StepShell step={2}>
      <div className="flex items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="text-xs font-bold uppercase tracking-widest text-ink/45">Live clock</p>
            <p className="text-sm font-bold text-ink">
              Question {index + 1} of {bank.questions.length}
            </p>
          </div>
        </div>
        <p className="rounded-bubble bg-ink/5 px-3 py-1 text-[11px] font-bold uppercase tracking-wider text-ink/50">
      ...
      <h3 className="mt-6 font-display text-xl font-bold leading-snug text-ink">{q.q}</h3>
            ...
            return (
              <button
                ...
                className={cn(
                  ...
                  selected === null && 'border-ink/12 bg-white text-ink/80 hover:border-cyan-snap/50 hover:bg-cyan-snap/5',
                  selected !== null && !isCorrect && !isWrong && 'border-ink/8 bg-white text-ink/40',
                )}
              >
                <span className="flex items-center gap-3">
                  <span className="grid h-7 w-7 shrink-0 place-items-center rounded-lg bg-ink/5 text-[11px] font-bold text-ink/50">
        ...
        {selected !== null && (
          <div className="mt-6 flex items-center justify-between gap-4">
            ...
            <Button size="md" variant="primary" onClick={next}>
              {index + 1 < bank.questions.length ? 'Next' : 'See results'}
