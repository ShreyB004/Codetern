  ...
  return (
    <div>
      ...
      <div className="mb-8 grid gap-4 lg:grid-cols-[1.5fr_1fr]">
        <div className="rounded-panel border border-ink/8 bg-white p-6 shadow-card">
          <div className="flex items-center gap-3">
            <span className="grid h-11 w-11 place-items-center rounded-2xl bg-ink text-neon">
              <Globe size={20} />
            </span>
            <div>
              <h3 className="font-display text-base font-bold text-ink">Global seat multiplier</h3>
              <p className="text-xs text-ink/50">Instantly scale maximum limits & remaining counts across every domain-batch.</p>
            </div>
          </div>
          <div className="mt-6 flex items-center gap-4">
            ...
            />
            <span className="w-16 shrink-0 rounded-xl bg-ink/5 px-3 py-1.5 text-center font-display text-sm font-bold text-ink">
              ×{factor.toFixed(1)}
            </span>
            <button
            ...
            <div key={p.id} className="rounded-panel border border-ink/8 bg-white p-5 shadow-card">
              <div className="mb-4 flex items-center gap-3">
                ...
                <div className="min-w-0">
                  ...
                  <p className="text-[11px] text-ink/45">{p.sub}</p>
                </div>
              </div>
...
function CellEditor({ duration, total, remaining, onChange }) {
  ...
  return (
    <div className="rounded-2xl border border-ink/8 bg-paper p-3.5">
      <div className="flex items-center justify-between">
        <span className="rounded-bubble bg-ink/5 px-2.5 py-0.5 text-[10px] font-bold uppercase tracking-wider text-ink/45">
          {duration} month{duration > 1 ? 's' : ''}
        </span>
        <span className="text-[11px] font-bold text-ink/50 tabular-nums">{remaining}/{total} left</span>
      </div>
      <div className="mt-3 grid grid-cols-2 gap-2">
        <label className="block">
          <span className="block text-[10px] font-semibold uppercase tracking-wider text-ink/40">Max seats</span>
          <input type="number" min="0" value={total} onChange={(e) => setTotal(e.target.value)} className="mt-1 w-full rounded-lg border border-ink/12 bg-white px-2.5 py-1.5 text-sm font-bold outline-none focus:border-cyan-snap/60" />
        </label>
        <label className="block">
          <span className="block text-[10px] font-semibold uppercase tracking-wider text-ink/40">Remaining</span>
          <input type="number" min="0" max={total} value={remaining} onChange={(e) => setRemaining(e.target.value)} className="mt-1 w-full rounded-lg border border-ink/12 bg-white px-2.5 py-1.5 text-sm font-bold outline-none focus:border-cyan-snap/60" />
        </label>
      </div>
