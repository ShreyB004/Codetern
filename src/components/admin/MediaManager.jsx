    ...
    <div>
      ...
      <div
        ...
        className={cn(
          ...
          drag ? 'border-cyan-snap/70 bg-cyan-snap/8' : 'border-ink/15 bg-white',
        )}
      >
        ...
        <div>
          <p className="font-display text-lg font-bold text-ink">Drop promotional images here</p>
          <p className="text-sm text-ink/50">or click to browse · PNG, JPG, WEBP · assigned to a layout slot</p>
        </div>
      ...
      <div className="grid gap-5 sm:grid-cols-2 xl:grid-cols-3">
        {media.map((m) => (
          <div key={m.id} className="group overflow-hidden rounded-panel border border-ink/8 bg-white shadow-card">
            <div className="relative aspect-[16/9] overflow-hidden bg-ink/5">
              ...
              <div className="absolute inset-0 flex items-center justify-center gap-2 bg-ink/60 opacity-0 backdrop-blur-sm transition group-hover:opacity-100">
                <button
                  ...
                  className="grid h-9 w-9 place-items-center rounded-xl bg-white text-ink transition hover:scale-110"
                  aria-label="Edit"
                >
                  <Pencil size={15} />
                </button>
            ...
            <div className="flex items-center justify-between gap-2 p-4">
              ...
              <span className="shrink-0 rounded-bubble bg-ink/5 px-2.5 py-1 text-[10px] font-bold uppercase tracking-wider text-ink/50">
                {m.kind}
              </span>
            </div>
          </div>
        ...
        <button
          ...
          className="grid min-h-[180px] place-items-center rounded-panel border border-dashed border-ink/20 text-ink/40 transition hover:border-cyan-snap/50 hover:text-cyan-snap"
        >
...
function EditModal({ asset, onClose, onSave }) {
  ...
  return (
    <div className="fixed inset-0 z-[110] grid place-items-center p-4">
      ...
      <div className="relative w-full max-w-md rounded-panel bg-white p-6 shadow-float">
        <h3 className="font-display text-lg font-bold text-ink">{asset ? 'Edit asset' : 'New asset slot'}</h3>
        <div className="mt-5 flex flex-col gap-4">
          <label className="block">
            <span className="mb-1.5 block text-xs font-semibold uppercase tracking-wide text-ink/50">Asset name</span>
            <input value={name} onChange={(e) => setName(e.target.value)} className="w-full rounded-xl border border-ink/12 px-4 py-2.5 text-sm outline-none focus:border-cyan-snap/60" />
          </label>
          <label className="block">
            <span className="mb-1.5 block text-xs font-semibold uppercase tracking-wide text-ink/50">Layout slot</span>
            <select value={slot} onChange={(e) => setSlot(e.target.value)} className="w-full rounded-xl border border-ink/12 px-4 py-2.5 text-sm outline-none focus:border-cyan-snap/60">
              {SLOTS.map((s) => <option key={s} value={s}>{s}</option>)}
            </select>
          </label>
        </div>
        <div className="mt-6 flex justify-end gap-3">
          <button onClick={onClose} className="rounded-full border border-ink/12 px-5 py-2.5 text-sm font-semibold text-ink/60 hover:bg-ink/5">Cancel</button>
