      ...
      <div className="relative mx-auto grid max-w-7xl items-center gap-14 px-5 pb-24 pt-16 lg:grid-cols-[1.1fr_1fr] lg:px-8 lg:pt-24">
        ...
        <div data-reveal>
          ...
          <h1 className="font-display text-balance text-[2.7rem] font-extrabold leading-[1.02] tracking-tight sm:text-6xl lg:text-7xl">
            Get some
            <br />
            <span className="text-gradient-snap">real work done</span>
            <br />
          ...
          <div className="mt-9 flex flex-wrap items-center gap-4">
            ...
            <Button size="lg" variant="lightGhost" magnetic={false} onClick={() => navigate('/domains')}>
              <PlayCircle size={17} />
              Explore domains
            </Button>
          </div>
          ...
          <div className="mt-10 flex flex-wrap gap-x-8 gap-y-3">
            {[
              ...
              <div key={label} className="flex items-center gap-2 text-sm text-white/55">
                <Icon size={15} className="text-cyan-snap" />
                {label}
              </div>
            ))}
          </div>
        ...
        <div className="relative hidden h-[520px] lg:block" data-enter-scale>
          {/* base panel */}
          <div className="absolute left-1/2 top-1/2 w-[380px] -translate-x-1/2 -translate-y-1/2 rounded-panel border border-white/10 bg-white/[0.04] p-6 shadow-float backdrop-blur-sm">
            <div className="flex items-center gap-3">
              <span className="grid h-11 w-11 place-items-center rounded-2xl bg-cyan-snap/15 text-cyan-snap">
                <Sparkles size={20} />
              </span>
              <div>
          ...
          <FloatingCard className="bottom-6 left-4" amplitude={10} duration={3.6} delay={0.3}>
            <div className="flex items-center gap-3 rounded-2xl border border-white/10 bg-white/[0.05] px-4 py-3 shadow-float backdrop-blur-md">
          ...
          <div className="pointer-events-none absolute -right-10 -top-10 h-40 w-40 rounded-full border border-dashed border-cyan-snap/30" />
        </div>
      </div>
    </section>
  )
