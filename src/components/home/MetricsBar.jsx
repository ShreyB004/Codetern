...
function Stat({ value, suffix = '', label }) {
  ...
  return (
    <div ref={ref} className="text-center" data-reveal>
      <p className="font-display text-4xl font-extrabold text-ink tabular-nums sm:text-5xl">
        {v.toLocaleString('en-IN')}
        {suffix}
      </p>
      <p className="mt-1 text-sm font-medium text-ink/50">{label}</p>
    </div>
  )
}
...
export function MetricsBar() {
  ...
  return (
    <section ref={scope} className="border-b border-ink/8 bg-paper">
