      ...
      <div className="mx-auto max-w-7xl px-5 lg:px-8">
        <div className="mb-12 flex flex-col items-start justify-between gap-4 lg:flex-row lg:items-end" data-reveal>
          ...
          <button
            ...
            className="group inline-flex items-center gap-1.5 text-sm font-semibold text-ink transition hover:text-cyan-snap"
          >
            See all programmes
          ...
          {featured.map((p, i) => {
            ...
            return (
              <button
                ...
                className="group relative flex flex-col overflow-hidden rounded-panel border border-ink/8 bg-white p-5 text-left shadow-card transition-all duration-500 hover:-translate-y-1.5 hover:shadow-float"
              ...
              >
                ...
                <h3 className="relative mt-4 font-display text-lg font-bold leading-snug text-ink">{p.title}</h3>
                <p className="relative mt-1 text-xs font-medium text-ink/45">{p.sub}</p>

                <div className="relative mt-4 flex flex-wrap gap-1.5">
                  {p.stack.slice(0, 4).map((s) => (
                    <span key={s} className="rounded-bubble bg-ink/5 px-2.5 py-1 text-[10px] font-semibold text-ink/60">
                      {s}
                    </span>
                  ))}
                </div>
