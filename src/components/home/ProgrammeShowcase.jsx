      ...
      <div className="relative mx-auto max-w-7xl px-5 lg:px-8">
        <div className="mb-12 flex flex-col items-start justify-between gap-4 lg:flex-row lg:items-end" data-reveal>
          ...
          <button
            ...
            className="group inline-flex min-h-11 items-center gap-1.5 rounded-full px-2 py-2 text-sm font-semibold text-ink dark:text-paper transition-colors focus-ring hover:text-violet-ink dark:hover:text-violet-deep"
          >
            See all programmes
          ...
          {featured.map((p, i) => {
            ...
            return (
              <button
                ...
                className="group relative flex flex-col overflow-hidden rounded-panel border border-ink/8 bg-white dark:border-paper/10 dark:bg-ink-soft p-5 text-left shadow-card transition-all duration-500 hover:-translate-y-1.5 hover:shadow-float focus-ring"
              ...
              >
                ...
                <div className="relative flex items-start justify-between gap-3">
                  <span
                    className="grid h-11 w-11 shrink-0 place-items-center rounded-2xl text-ink/80 dark:text-paper"
                ...
                <span role="heading" aria-level="3" className="relative mt-4 block font-display text-lg font-bold leading-snug text-ink dark:text-paper">{p.title}</span>
                <span className="relative mt-1 block text-xs font-medium text-ink/60 dark:text-paper/60">{p.sub}</span>
                ...
                <div className="relative mt-5 flex items-center justify-between border-t border-ink/8 dark:border-paper/8 pt-4">
                  <span className="flex items-center gap-1.5 text-xs font-semibold text-ink/60 dark:text-paper/60">
                    <Check size={13} className="text-mint-deep dark:text-mint" />
                    {dur}-month track
                  </span>
                  <span className="text-xs font-bold text-violet-ink opacity-0 transition group-hover:opacity-100 group-focus-visible:opacity-100 dark:text-violet-deep">
                    Reserve →
                  </span>
                </div>
              </button>
